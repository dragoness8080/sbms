...
if($_GPC['ac']=='getDate'){
	...
	$list=pdo_fetchall("select * from ".tablename('sbms_room')." where `uniacid`='$uniacid'  and seller_id= '$seller_id' order by id desc");
	...
	foreach ($list as $key => $value) {
		...
		for($i = 0; $i <= $pagesize; $i++){
			...
			$rplist=pdo_get('sbms_roomprice',array('dateday'=>$dateday1,'rid'=>$rid));
        			//print_r($rplist);
			if($rplist['id']){
				$pricearr[$i]['mprice']=$rplist['mprice'];
				$pricearr[$i]['rid']=$value['id'];
				$pricearr[$i]['pid']=$rplist['id'];
				$pricearr[$i]['dateday']=date('Y-m-d', $date_array[$i]['time']);
			}else{
				$pricearr[$i]['mprice']=$value['price'];
				$pricearr[$i]['dateday']=date('Y-m-d', $date_array[$i]['time']);
				$pricearr[$i]['rid']=$value['id'];
			}
	...
	$data['result'] = 1;
	ob_start();
	include $this->template('web/roompricelist');
...
include $this->template('web/dlprice');