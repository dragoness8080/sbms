...
$stores=pdo_getall('sbms_seller',array('uniacid'=>$_W['uniacid'],'state'=>2));
if ($operation == 'display') {
    $item = pdo_fetch("SELECT * FROM ". tablename('sbms_system') . " WHERE uniacid = :weid", array('weid' => $_W['uniacid']));
   if($_GPC['color']){
        $color=$_GPC['color'];
    }else{
         $color="#182E8F";
    ...
    if (checksubmit()) {   
        ...
        if (empty($item)) {
            pdo_insert('sbms_system', $data, $replace = false);
            message('操作成功!', $this->createWebUrl('settings', array('op' => 'display', 'storeid' => $storeid)), 'success');
        } else {
            pdo_update('sbms_system', $data, array('id' => $item['id']));
            message('操作成功!', $this->createWebUrl('settings', array('op' => 'display', 'storeid' => $storeid)), 'success');
        }
    }
}
