...
$GLOBALS['frames'] = $this->getMainMenu();
	$info = pdo_get('sbms_jfgoods',array('uniacid' => $_W['uniacid'],'id'=>$_GPC['id']));
	$type = pdo_getall('sbms_jftype',array('uniacid' => $_W['uniacid']));
		if(checksubmit('submit')){
			...
			if($_GPC['id']==''){				
				$res=pdo_insert('sbms_jfgoods',$data);
				if($res){
				...
				$res = pdo_update('sbms_jfgoods', $data, array('id' => $_GPC['id']));
				if($res){
