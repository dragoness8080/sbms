...
// $user = pdo_fetchall("SELECT * FROM " . tablename('sbms_seller') . " WHERE uniacid= :weid  and state=2 ORDER BY id DESC", array(':weid' => $_W['uniacid']), 'id');
    $id = intval($_GPC['id']);
    //echo $id;die;
    $user_id = intval($_GPC['user_id']);
    if (!empty($id)) {
        $account = pdo_fetch("SELECT * FROM " . tablename('sbms_account') . " WHERE weid = :weid AND id=:id ORDER BY id DESC", array(':weid' => $_W['uniacid'], ':id' => $id));
    }
    if (!empty($account)) {
            ...
            if ($uid > 0) {
             ...
             //operator
                ...
                pdo_insert('users_permission', array(
                    ...
                    'url' => '',
                    'type' => 'sbms',
                   'permission' => 'sbms_menu_store'
                ));

                pdo_insert('sbms_account', array(
                    'uid' => $uid,
                    'weid' => $_W['uniacid'],
                    'storeid' => $seller_id,
