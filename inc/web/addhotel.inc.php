...
$list = pdo_get('sbms_seller',array('id'=>$_GPC['id']));
$user=pdo_getall('sbms_user',array('uniacid'=>$_W['uniacid']));
...
$userinfo2=pdo_get('sbms_user',array('id'=>$list['user_id']));
if($list['img']){
...
if(checksubmit('submit')){
  ...
  $data['special_offer'] = $_GPC['special_offer'];
  if($_GPC['id']==''){
    $data['owner']=1;
     $data['state']=2;
       $data['time']=time();
    ...
    $res = pdo_update('sbms_seller', $data, array('id' => $_GPC['id']));
    if($res){
