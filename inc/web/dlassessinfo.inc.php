...
$sql="select a.* ,b.name from " . tablename("sbms_assess") . " a"  . " left join " . tablename("sbms_user") . " b on b.id=a.user_id where a.uniacid=:uniacid and a.id=:id";
$list=pdo_fetch($sql, array(':uniacid'=>$_W['uniacid'],':id'=>$_GPC['id']));
if (checksubmit('submit')) {
	$data['content']=$_GPC['content'];
	$data['reply']=$_GPC['reply'];
	$res=pdo_update("sbms_assess",$data,array('id'=>$_GPC['id']));
	if($res){
