...
class SbmsModuleWxapp extends WeModuleWxapp {
	...
	public function doPageLogin(){
		global $_GPC, $_W;
		$openid=$_GPC['openid'];
		$res=pdo_get('sbms_user',array('openid'=>$openid,'uniacid'=>$_W['uniacid']));
		if($res){
			$user_id=$res['id'];
			...
			$res = pdo_update('sbms_user', $data, array('id' =>$user_id));
			$sql="select a.*,b.name as level_name,b.discount  from " . tablename("sbms_user") ." a left join".tablename('sbms_level')." b on a.level_id=b.id WHERE  a.openid=:openid and a.uniacid=:uniacid";
			...
			$data['join_time']=time();
			$data['type']=1;
			$res2=pdo_insert('sbms_user',$data);
			$sql="select a.*,b.name as level_name,b.discount  from " . tablename("sbms_user") ." a left join".tablename('sbms_level')." b on a.level_id=b.id WHERE  a.openid=:openid and a.uniacid=:uniacid";
			$user=pdo_fetch($sql,array(':openid'=>$openid,'uniacid'=>$_W['uniacid'] ));
			echo json_encode($user);
		}
...
//系统设置
	...
	public function doPageGetAd(){
      ...
      //  $res=pdo_getall('zh_gjhdbm_ad',array('uniacid'=>$_W['uniacid'],'status'=>1,'type'=>$_GPC['type']));
		...
		$data[':uniacid']=$_W['uniacid'];
		$data[':type']=$_GPC['type'];
		$sql="select *  from " . tablename("sbms_ad") .$where." order by id  desc ";
...
//导航列表
	...
	public function  doPageGetNav(){
		global $_GPC, $_W;
		$res=pdo_getall('sbms_nav',array('uniacid'=>$_W['uniacid'],'status'=>1),array(),'','orderby asc');
		echo json_encode($res);
	}
...
//酒店入住
	...
	public  function doPageSaveHotelApply(){
		...
		$data['sq_time']=time();
		$data['uniacid']=$_W['uniacid'];
		$rst=pdo_get('sbms_seller',array('user_id'=>$_GPC['user_id'],'uniacid'=>$_W['uniacid']));
		if($rst){			
			$res = pdo_update('sbms_seller', $data, array('user_id' => $_GPC['user_id']));
		}else{     
			$res=pdo_insert('sbms_seller',$data);  			
		}
		if($res){
			echo '1';
		}else{
	...
	public function doPageCheckRz(){
		global $_W, $_GPC;
		$res=pdo_get('sbms_seller',array('user_id'=>$_GPC['user_id'],'uniacid'=>$_W['uniacid']),'state');
		echo json_encode($res);
	}
...
public function doPageJiemi(){
	global $_W, $_GPC;
	$res=pdo_get('sbms_system',array('uniacid'=>$_W['uniacid']));
	include_once  IA_ROOT . "/addons/sbms/wxBizDataCrypt.php";
	$appid = $res['appid'];
...
public function doPageJdList(){
	...
	$sql="select *  from " . tablename("sbms_seller") .$where." order by scort	asc ";
...
public function doPagePjDetails(){
	global $_GPC, $_W;
	$res=pdo_get("sbms_seller",array('id'=>$_GPC['seller_id']));
	pdo_update('sbms_seller',array('ll_num +='=>1),array('id'=>$_GPC['seller_id']));
	echo json_encode($res);
}
...
public function doPageRoomList(){
	...
	$sql="select *  from " . tablename("sbms_room") .$where." order by sort asc ";
	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$list=pdo_fetchall($select_sql,$data);
	echo json_encode($list);
}
...
public  function doPageRoomDetails(){
	global $_GPC, $_W;
	$res=pdo_get("sbms_room",array('id'=>$_GPC['room_id']));
	echo json_encode($res);
}
...
public function doPageGetSponsorCoupon(){
	...
	$time=strtotime($time);
	$sql=" select * from".tablename('sbms_coupons')." where uniacid=:uniacid  and lq_num < number and unix_timestamp(end_time) >= $time 
	 and id not in (select coupons_id from".tablename('sbms_usercoupons')." where user_id={$_GPC['user_id']} and uniacid={$_W['uniacid']}) order by id desc";
	//echo $sql;die;
	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
...
// 	$time=date("Y-m-d",time());
// 	$time=strtotime($time);
// 	$sql=" select * from".tablename('sbms_coupons')." where uniacid=:uniacid  and seller_id=:seller_id and lq_num < number and unix_timestamp(end_time) >= $time order by id desc";
...
public function doPageCouponDetails(){
	global $_GPC, $_W;
	$res=pdo_get("sbms_coupons",array('id'=>$_GPC['coupon_id']));
	echo json_encode($res);
}
...
public function doPageReceiveCoupons(){
	global $_GPC, $_W;	
	$rst=pdo_get('sbms_usercoupons',array('user_id'=>$_GPC['user_id'],'coupons_id'=>$_GPC['coupons_id'],'uniacid'=>$_W['uniacid']));
	if(!$rst){
	$data['user_id']=$_GPC['user_id'];
	...
	$data['time']=time();
	$data['uniacid']=$_W['uniacid'];
	$res=pdo_insert('sbms_usercoupons',$data);
	if($res){
	//修改券的数量
	pdo_update('sbms_coupons',array('lq_num +='=>1),array('id'=>$_GPC['coupons_id']));
	echo '1';
	}else{
		echo '2';
	}
...
//生成订单
	...
	while ($dt_start<$dt_end){ 
		$dateday=$dt_start;
		$res=pdo_get('sbms_roomnum',array('rid'=>$rid,'dateday'=>$dateday));
		$surplus=$res['nums'];
		if(!$res['id']){
			$surplus=pdo_getcolumn('sbms_room',array('id'=>$rid),'total_num');
		}
	...
	if($str){
		...
		$res=pdo_insert('sbms_order',$data);
		$order_id=pdo_insertid();
		if($_GPC['hb_id']){//使用积分红包
			pdo_update('sbms_jfhb',array('state'=>2),array('id'=>$_GPC['hb_id']));
		}
		if($_GPC['type']==3){
		file_get_contents("".$_W['siteroot']."/app/index.php?i=".$_W['uniacid']."&c=entry&a=wxapp&do=print&m=sbms&order_id=".$order_id);
		if($_GPC['coupons_id']){//使用优惠券
		    pdo_update('sbms_usercoupons',array('state'=>2,'sy_time'=>time()),array('coupons_id'=>$_GPC['coupons_id'],'user_id'=>$_GPC['user_id']));
		}
	}
...
public function doPagePay(){
	global $_W, $_GPC;
	include IA_ROOT.'/addons/sbms/wxpay.php';
	$res=pdo_get('sbms_system',array('uniacid'=>$_W['uniacid']));
	$appid=$res['appid'];
	...
	$root=$_W['siteroot'];
	pdo_update('sbms_order',array('out_trade_no'=>$out_trade_no),array('id'=>$_GPC['order_id']));  
...
public function doPageYePay(){
	...
	$data['time']=time();
	$data['uniacid']=$_W['uniacid'];
	$res=pdo_insert('sbms_recharge',$data);
	if($res){
		pdo_update('sbms_order',array('status'=>2),array('id'=>$_GPC['order_id']));
		if($orderInfo['coupons_id']){//使用优惠券
		    pdo_update('sbms_usercoupons',array('state'=>2,'sy_time'=>time()),array('coupons_id'=>$orderInfo['coupons_id'],'user_id'=>$orderInfo['user_id']));
		}
		pdo_update('sbms_user',array('balance -='=>$orderInfo['total_cost']),array('id'=>$orderInfo['user_id']));
		//echo "".$_W['siteroot']."/app/index.php?i=".$_W['uniacid']."&c=entry&a=wxapp&do=print&m=sbms&order_id=".$_GPC['order_id'];die;
		echo file_get_contents("".$_W['siteroot']."/app/index.php?i=".$_W['uniacid']."&c=entry&a=wxapp&do=print&m=sbms&order_id=".$_GPC['order_id']);//打印小票die;
		file_get_contents("".$_W['siteroot']."app/index.php?i=".$_W['uniacid']."&c=entry&a=wxapp&do=Sms&m=sbms&seller_id=".$orderInfo['seller_id']);//短信
		echo '1';
	}else{
		echo '2';
	}
...
public function doPageMyCoupons(){
	...
	$sql="select b.*,a.id as lq_id,a.state  from " . tablename("sbms_usercoupons")." a left join".tablename('sbms_coupons')." b on a.coupons_id=b.id" .$where." order by id desc ";
	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$list=pdo_fetchall($select_sql,$data);
	echo json_encode($list);
}
...
public function  doPageSaveAssess(){
	global $_W, $_GPC;
	$res2=pdo_get('sbms_system',array('uniacid'=>$_W['uniacid']));
	$data['seller_id']=$_GPC['seller_id'];
	...
	$data['time']=time();
	$data['uniacid']=$_W['uniacid'];
	$res=pdo_insert('sbms_assess',$data);
	$assess_id=pdo_insertid();
	if($res){
		echo '1';
		//完成订单
	pdo_update('sbms_order',array('status'=>4),array('id'=>$_GPC['order_id']));
    ...
    pdo_insert('sbms_score',$data2);
    pdo_update('sbms_user',array('score +='=>$res2['pl_score']),array('id'=>$_GPC['user_id']));
	}else{
		echo '2';
	}
}
...
public function doPageAssessList(){
	...
	$pageindex = max(1, intval($_GPC['page']));
	$pagesize=10;
	$sql=" select a.*,b.name,b.img as logo from".tablename('sbms_assess')." a left join ".tablename('sbms_user')." b on a.user_id=b.id  where a.uniacid=:uniacid  and a.seller_id=:seller_id order by a.id desc";
	//echo $sql;die;
	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
...
//短信通知
  public function doPageSms(){
  	global $_W, $_GPC;
  	$res=pdo_get('sbms_notice',array('uniacid'=>$_W['uniacid'],'seller_id'=>$_GPC['seller_id']));
  	$tpl_id=$res['tpl_id'];
  	$tel=$res['js_tel'];
  	$key=$res['appkey'];
...
public function doPageOrderDetails(){
	global $_GPC, $_W;
	$res=pdo_get('sbms_order',array('id'=>$_GPC['order_id']));
	echo json_encode($res);
}
...
public function doPageCancelOrder(){
	global $_GPC, $_W;
	$res=pdo_update('sbms_order',array('status'=>3),array('id'=>$_GPC['order_id']));
	if($res){
			if($_GPC['hb_id']){//使用积分红包
				pdo_update('sbms_jfhb',array('state'=>1),array('id'=>$_GPC['hb_id']));
			}
			echo '1';
		}else{
			echo '2';
...
public function doPageApplyOrder(){
	global $_GPC, $_W;
	$res=pdo_update('sbms_order',array('status'=>6),array('id'=>$_GPC['order_id']));
	if($res){
		echo '1';
	}else{
		echo '2';
...
public function doPageGetRoomCost(){
	...
	while ($dt_start<$dt_end){ 
		$dateday=$dt_start;
		$res1=pdo_get('sbms_roomprice',array('rid'=>$rid,'dateday'=>$dateday));
		$pricelist[$i]['dateday']=date('m月d日',$dateday);
		if($res1['mprice']){
			$pricelist[$i]['mprice']=$res1['mprice'];
		}else{
			$mprice=pdo_getcolumn('sbms_room',array('id'=>$rid),'price');
...
public  function doPageRefund(){
	...
	$refund_order =pdo_get('sbms_order',array('id'=>$_GPC['order_id']));  
    ...
    public function doPageMessage(){
	 ...
	 //设置与发送模板信息
    	function set_msg($_W){
    		...
    		$res=pdo_get('sbms_system',array('uniacid'=>$_W['uniacid']));
    		$sql="select * from " . tablename("sbms_order")." WHERE id=:order_id  ";
...
//分类列表
    public function doPageJfTypeList(){
    	global $_W, $_GPC;
    	$res=pdo_getall('sbms_jftype',array('uniacid'=>$_W['uniacid']),array(),'','num asc');
    	echo json_encode($res);
    }
    ...
    public  function  doPageJfGoodsList(){
    	...
    	$sql="select *  from " . tablename("sbms_jfgoods") .$where." order by num asc ";
    	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
    	$list=pdo_fetchall($select_sql,$data);
    	echo json_encode($list);
    ...
    public function doPageGoodsDetails(){
    	global $_W, $_GPC;
    	$res=pdo_get('sbms_jfgoods',array('id'=>$_GPC['id']));
    	echo json_encode($res);
    }
    ...
    public function doPageExchange(){
        ...
        $res=pdo_insert('sbms_jfrecord',$data);
      if($res){
      	pdo_update('sbms_jfgoods',array('number -='=>1),array('id'=>$_GPC['good_id']));
          if($_GPC['type']==1){//虚拟红包
          ...
          pdo_insert('sbms_score',$data3); 
          pdo_update('sbms_user',array('score -='=>$_GPC['integral']),array('id'=>$_GPC['user_id']));
          echo '1';
      }else{
      	echo '2';
      }
 ...
 //兑换明细
  public function doPageDhmx(){
  	...
  	$sql="select *  from " . tablename("sbms_jfrecord") .$where." order by id desc ";
  	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
  	$list=pdo_fetchall($select_sql,$data);
  	echo json_encode($list);
  ...
  public function doPageMyScoreDetails(){
  	...
  	$data[':user_id']=$_GPC['user_id'];
  	$where=" where uniacid=:uniacid and user_id=:user_id ";
  	$sql="select *  from " . tablename("sbms_score") .$where." order by id desc ";
  	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
  	$list=pdo_fetchall($select_sql,$data);
  	echo json_encode($list);
  }
...
//我的可用红包
  public function doPageMyHb(){
  	...
  	$sql="select *  from " . tablename("sbms_jfhb") .$where." order by id desc ";
  	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
  	$list=pdo_fetchall($select_sql,$data);
  	echo json_encode($list);  	
...
global $_W, $_GPC;
	...
	while ($dt_start<$dt_end){ 
		$dateday=$dt_start;
		$res1=pdo_get('sbms_roomnum',array('rid'=>$rid,'dateday'=>$dateday));
		$pricelist[$i]['dateday']=date('m月d日',$dateday);
		if($res1){
			$pricelist[$i]['nums']=$res1['nums'];
		}else{
			$mprice=pdo_getcolumn('sbms_room',array('id'=>$rid),'total_num');
...
public function doPageMemberList(){
	global $_W, $_GPC;
	$res=pdo_getall('sbms_level',array('uniacid'=>$_W['uniacid']),array(),'','value asc');
	echo json_encode($res);
}
...
public function doPageHtLogin(){
	...
	if(!empty($record)) {
		$account = pdo_fetch("SELECT * FROM " . tablename("sbms_account") . " WHERE status=2 AND uid=:uid ORDER BY id DESC LIMIT 1", array(':uid' => $record['uid']));
		if (!empty($account)) {
			$storeid = $account['storeid'];
			$data[':uniacid']=$_W['uniacid'];
			$data[':seller_id']=$storeid;
			//获取酒店信息
			$sellerInfo=pdo_get('sbms_seller',array('id'=>$storeid),array('name','ewm_logo','ll_num'));
			//今日订单
			$order=" select count(id) as total from".tablename('sbms_order')." where uniacid=:uniacid and seller_id=:seller_id and  time>=".$time." and time<".$mttime."";
			$total=pdo_fetch($order,$data);
			//今日销售额
			$moneysql=" select sum(total_cost) as total_money, sum(ytyj_cost) as yj_money  from".tablename('sbms_order')." where uniacid=:uniacid  and seller_id=:seller_id and status in (2,4,5,6,8)  and time>=".$time." and time<".$mttime."";
			$jrmoney=pdo_fetch($moneysql,$data);
			//本月销售额
			$bysql=" select sum(total_cost) as total_money, sum(ytyj_cost) as yj_money  from".tablename('sbms_order')." where uniacid=:uniacid  and seller_id=:seller_id and status in (2,4,5,6,8)  and time>=".$month;
			$bymoney=pdo_fetch($bysql,$data);
			//订单统计
			$sql2="select count( case when status=1 then 1 end) as dzf, count( case when status=4 then 1 end) as ywc, count( case when status=7 then 1 end) as ytk from  ".tablename('sbms_order')." where uniacid=:uniacid  and seller_id=:seller_id ";
			$tjorder=pdo_fetch($sql2,$data);
			//昨日营业额
			$zrsql=" select sum(total_cost) as total_money, sum(ytyj_cost) as yj_money  from".tablename('sbms_order')." where uniacid=:uniacid  and seller_id=:seller_id and status in (2,4,5,6,8)  and time>=".$zttime." and time<".$time."";
			$zrmoney=pdo_fetch($zrsql,$data);
			//总营业额
			$totalsql=" select sum(total_cost) as total_money, sum(ytyj_cost) as yj_money  from".tablename('sbms_order')." where uniacid=:uniacid  and seller_id=:seller_id and status in (2,4,5,6,8) ";
...
public function doPageStoreWxLogin(){
	global $_GPC, $_W;
	$res=pdo_get('sbms_seller',array('user_id'=>$_GPC['user_id'],'state'=>2));
	if($res){
		$storeid=$res['id'];
		$time=strtotime(date("Y-m-d"));
		$mttime=strtotime(date("Y-m-d",strtotime("+1 day")));
...
public function doPageChangeOrder(){
	global $_W, $_GPC;
	$res=pdo_update('sbms_order',array('status'=>$_GPC['status']),array('id'=>$_GPC['order_id']));
	if($res){
		if($_GPC['status']==5){
		$rst=file_get_contents("".$_W['siteroot']."/app/index.php?i=".$_W['uniacid']."&c=entry&a=wxapp&do=UpdRz&m=sbms&id=".$_GPC['order_id']);
		}
		echo '1';
	}else{
		echo'2';
...
public function doPageEditRoom(){
	...
	$res = pdo_update('sbms_room', $data, array('id' =>$_GPC['room_id']));
	if($res){
		echo '1';
	}else{
		echo '2';
...
public function doPageDeleteRoom(){	
	global $_W, $_GPC;
	$res=pdo_delete('sbms_room',array('id'=>$_GPC['room_id']));
	if($res){
		echo '1';
	}else{
		echo'2';
...
public function doPageChangeRoom(){
	global $_W, $_GPC;
	$res=pdo_update('sbms_room',array('state'=>$_GPC['state']),array('id'=>$_GPC['room_id']));
	if($res){
		echo '1';
	}else{
		echo'2';
...
public function doPageEditRoomPrice(){
	...
	if(!$res['id']){
		$rst=pdo_insert('sbms_roomprice',$data);
	}else{
		$rst=pdo_update('sbms_roomprice',array('mprice'=>$data['mprice']),array('id'=>$res['id']));
	}
	if($rst){
		echo '1';
	}else{
...
public function doPageEditRoomNum(){
	...
	if(!$res['id']){
		$rst=pdo_insert('sbms_roomnum',$data);
	}else{
		$rst=pdo_update('sbms_roomnum',array('nums'=>$data['nums']),array('id'=>$res['id']));
	}
	if($rst){
		echo '1';
	}else{
...
public function doPageTxMoney(){
	...
	$data2[':uniacid']=$_W['uniacid'];
	  //总营业额
	$totalsql=" select sum(total_cost) as total_money, sum(ytyj_cost) as yj_money  from".tablename('sbms_order')." where uniacid=:uniacid  and seller_id=:seller_id and status in (4,5,8) and type !=3 ";
	$totalmoney=pdo_fetch($totalsql,$data2);
	$sql2 = "select sum(tx_cost) as total from " . tablename("sbms_withdrawal")." WHERE  seller_id=:seller_id and state in (1,2) and uniacid=:uniacid ";
...
public function doPageSaveTxApply(){
	...
	$data['uniacid']=$_W['uniacid'];
	$data['time']=date("Y-m-d H:i:s");
	$res=pdo_insert('sbms_withdrawal',$data);
	if($res){
		echo '1';
	}else{
		echo '2';
...
public function doPageSellerTxList(){
    ...
    $sql="SELECT * FROM ".tablename('sbms_withdrawal') .  $where." ORDER BY time DESC";
  	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
  	$list=pdo_fetchall($select_sql,$data);
  	echo json_encode($list);
}
...
public function doPageReplyAssess(){
	...
	$res=pdo_update('sbms_assess',$data,array('id'=>$_GPC['assess_id']));
	if($res){
		echo '1';
	}else{
		echo '2';
...
public  function doPageGetFxSet(){
	 global $_W, $_GPC;
	 $res=pdo_get('sbms_fxset',array('uniacid'=>$_W['uniacid']));
	 echo json_encode($res);
}
...
//申请分销商
  public function doPageDistribution(){
    global $_W, $_GPC;
    pdo_delete('sbms_distribution',array('user_id'=>$_GPC['user_id']));
    $set=pdo_get('sbms_fxset',array('uniacid'=>$_W['uniacid']));
    $data['user_id']=$_GPC['user_id'];
    ...
    $data['uniacid']=$_W['uniacid'];
    $res=pdo_insert('sbms_distribution',$data);
    $sq_id=pdo_insertid();
    if($res){
      $fx=pdo_get('sbms_fxuser',array('fx_user'=>$_GPC['user_id']));
       if($set['is_fx']==1 and !$fx){
        pdo_insert("sbms_fxuser",array('user_id'=>0,'fx_user'=>$_GPC['user_id'],'time'=>time()));
       }
     echo  $sq_id;
    }else{
      echo '申请失败!';
  ...
  public function doPageMySx(){
      global $_W, $_GPC;
     $sql="select a.* ,b.name from " . tablename("sbms_fxuser") . " a"  . " left join " . tablename("sbms_user") . " b on b.id=a.user_id   WHERE a.fx_user=:fx_user ";
  ...
  public function doPageMyDistribution(){
      global $_W, $_GPC;
      $res=pdo_get('sbms_distribution',array('user_id'=>$_GPC['user_id']));
      echo json_encode($res);
  }
...
//绑定分销商
  public function doPageBinding(){
      global $_W, $_GPC;
      $res=pdo_get('sbms_fxuser',array('fx_user'=>$_GPC['fx_user']));//已绑定
      $res2=pdo_get('sbms_fxuser',array('user_id'=>$_GPC['fx_user'],'fx_user'=>$_GPC['user_id']));//已绑定成下线
      if($_GPC['user_id']==$_GPC['fx_user']){
        echo '自己不能绑定自己';
      }elseif($res || $res2){
        echo '不能重复绑定';
      }else{
        $res3=pdo_insert('sbms_fxuser',array('user_id'=>$_GPC['user_id'],'fx_user'=>$_GPC['fx_user'],'time'=>time()));
        if($res3){
          echo  '1';
        }else{
          echo  '2';
...
public function doPageMyTeam(){
    global $_W, $_GPC;
    $sql="select a.* ,b.name ,b.img from " . tablename("sbms_fxuser") . " a"  . " left join " . tablename("sbms_user") . " b on b.id=a.fx_user   WHERE a.user_id=:user_id order by id DESC";
    $res=pdo_fetchall($sql,array(':user_id'=>$_GPC['user_id']));
    $res2=array();
    for($i=0;$i<count($res);$i++){
        $sql2="select a.* ,b.name ,b.img from " . tablename("sbms_fxuser") . " a"  . " left join " . tablename("sbms_user") . " b on b.id=a.fx_user   WHERE a.user_id=:user_id order by id DESC";
...
public function doPageCountCommission(){
	global $_W, $_GPC;
	$sql2="select sum(case when state=1 then money else 0 end ) as dj ,sum( case when state=2 then money else 0 end ) as yx from  ".tablename('sbms_earnings')." where uniacid=:uniacid  and user_id=:user_id ";
	//echo $sql2;die;
	$money=pdo_fetch($sql2,array('uniacid'=>$_W['uniacid'],':user_id'=>$_GPC['user_id']));
	$sql3=" select sum(tx_cost) as tx_money from ".tablename('sbms_commission_withdrawal')." where  user_id=:user_id and state in (1,2)";
...
public function doPageYjlist(){
	global $_GPC, $_W;
  	...
  	$pagesize=30;
  	$data[':user_id']=$_GPC['user_id'];
  	$sql="  select id,money,note,time from ".tablename('sbms_earnings')." where user_id=:user_id and state=2  union all select id,tx_cost as money ,note,time from".tablename('sbms_commission_withdrawal')." where user_id=:user_id order by time desc";
  	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;	
  	$list=pdo_fetchall($sql,$data);
  	echo json_encode($list);  	
...
public function doPageGetYjTxSet(){
	global $_GPC, $_W;
	$res=pdo_get('sbms_fxtxset',array('uniacid'=>$_W['uniacid']));
	echo json_encode($res);
}
...
public function doPageYjtx(){
	...
	$data['user_id']=$_GPC['user_id']; 
	$data['note']='提现';     
	$res=pdo_insert('sbms_commission_withdrawal',$data);
	if($res){
		echo '1';
	}else{
		echo '2';	
...
public function doPageRenewUser(){
	...
	$res=pdo_update('sbms_user',$data,array('id'=>$_GPC['user_id']));
	if($res){
		echo '1';
	}else{
		echo '2';
...
public function doPageMyCost(){
	global $_W, $_GPC;
	$total=pdo_get('sbms_order', array('user_id'=>$_GPC['user_id'],'status '=>array(4,5,8)), array('sum(total_cost) as total_money','sum(ytyj_cost) as yt_money'));	
  ...
  public function doPageOrderCode(){
      ...
      function  getCoade($order_id){
        ...
        function set_msg($order_id){
         ...
         $data2=array(
          "scene"=>$order_id,
          "page"=>"sbms/pages/logs/Workbench",
          "width"=>100
          );
         $data2 = json_encode($data2);
         $url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=".$access_token."";
...
//二维码确认入住
  public function doPageSmRz(){
  	global $_W, $_GPC;
  	$orderInfo=pdo_get('sbms_order',array('id'=>$_GPC['order_id']),'seller_id');
  	if($orderInfo['seller_id']==$_GPC['seller_id']){
  		$res=pdo_update('sbms_order',array('status'=>5),array('id'=>$_GPC['order_id']));
  		if($res){
  			echo '1';
  		}else{
  			echo'2';
...
//充值活动
      public function doPageCzhd(){
       global $_W, $_GPC;
       $res=pdo_getall('sbms_czhd',array('uniacid'=>$_W['uniacid']),array(),'','full DESC');
       echo json_encode($res);
     }
...
public function doPagePay2(){
	...
	$out_trade_no = $mch_id. time();
	$root=$_W['siteroot'];
	pdo_update('sbms_recharge',array('out_trade_no'=>$out_trade_no),array('id'=>$_GPC['cz_id']));  
	$total_fee =$_GPC['money'];
	if(empty($total_fee)) 
	{
		$body = "订单付款";
...
public function doPageSaveRecharge(){
	...
	$data['time']=time();
	$data['uniacid']=$_W['uniacid'];
	$res2=pdo_insert('sbms_recharge',$data); 
	$cz_id=pdo_insertid();
	if($res2){
		echo  $cz_id;
	}else{
...
public function doPageYelist(){
	global $_GPC, $_W;
  	...
  	$pagesize=30;
  	$data[':user_id']=$_GPC['user_id'];
  	$sql="  select id,cz_money,zs_money,note,time from ".tablename('sbms_recharge')." where user_id=:user_id and state=2 ";
  	$select_sql=$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;	
  	$list=pdo_fetchall($sql,$data);
  	echo json_encode($list);  	
...
public function doPageUpdRz(){
	...
	function getScore($order_id){   
		global $_W, $_GPC;
		$orderInfo=pdo_get('sbms_order',array('id'=>$order_id));
		$scoreInfo=pdo_get('sbms_system',array('uniacid'=>$_W['uniacid']));
	...
	function saveScore($order_id,$user_id,$score){   
		...
		$data['time']=time();
		$data['uniacid']=$_W['uniacid'];
		$rst=pdo_insert('sbms_score',$data);
		return $rst;

	}
 function roomNum($order_id){
		global $_W, $_GPC;
		$order=pdo_get('sbms_order',array('id'=>$order_id));
        //修改房间数量
		...
		while ($dt_start<$dt_end){
			...
			if($res['id']){
				...
				$uniacid=$_W['uniacid'];    
				$roomArr=pdo_get('sbms_room',array('id'=>$order['room_id'],'uniacid'=>$order['uniacid']),array('total_num','id'));
				$nums=$roomArr['total_num']-$order['num'];
				pdo_insert('sbms_roomnum',array('nums'=>$nums,'rid'=>$roomArr['id'],'dateday'=>$dateday));
			}
			$dt_start = strtotime('+1 day',$dt_start);
		}
	}
