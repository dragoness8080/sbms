...
//		message("欢迎回来，{$record['username']}。", $forward);
		...
		pdo_update("sbms_account", $data, array('id' => $record['id']));

		message("欢迎回来，{$record['username']}！", url('site/entry/stores', array('m' => 'sbms', 'id' => $storeid,'uid'=>$record['uid'], 'do' => 'dlstatistics')), 'success');
	} else {
		if (empty($failed)) {
